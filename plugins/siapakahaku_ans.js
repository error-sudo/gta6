...
handler.before = async function (m) {
    ...
    if (m.quoted.id == this.siapakahaku[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            global.DATABASE.data.users[m.sender].exp += this.siapakahaku[id][2]
            await this.sendButton(m.chat, `*Benar!* +${this.siapakahaku[id][2]} XP`, 'CyberDark', 'Siapakah Aku', '.siapaaku')

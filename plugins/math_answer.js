...
handler.before = async function (m) {
  ...
  try {
    ...
    if (m.text == math.result) {
      ...
      delete this.math[id]
      await this.sendButton(m.chat, `*Benar!* +${math.bonus} XP`, 'CyberDark', `${math.mode.toUpperCase()}`, `.math ${math.mode}`, m)
    } else {
      if (--this.math[id][2] == 0) {
        clearTimeout(this.math[id][3])
        delete this.math[id]
        await this.sendButton(m.chat, `*Kesempatan habis!*\nJawaban: *${math.result}*`, 'CyberDark', `${math.mode.toUpperCase()}`, `.math ${math.mode}`)

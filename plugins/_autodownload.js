...
handler.all = async function (m, { isPrems }) {
    ...
    if (/^.*tiktok/i.test(m.text)) {
        ...
        await this.sendFile(m.chat, json.nowm, '', 'CyberDark', m)
    }
    ...
    if (/^.*cocofun/i.test(m.text)) {
        ...
        await m.reply(wait)
        // m.reply(util.format(json))
        await this.sendFile(m.chat, json.download, '', 'CyberDark', m)
    }
    ...
    if (/^.*instagram.com\/(p|reel|tv)/i.test(m.text)) {
        igdl(url).then(async res => {
            ...
            for (let { downloadUrl, type } of json) {
                this.sendFile(m.chat, downloadUrl, 'ig' + (type == 'image' ? '.jpg' : '.mp4'), 'CyberDark', m, 0, { thumbnail: await (await fetch(downloadUrl)).buffer() })
            }
        }).catch(_ => _)
    }
    ...
    if (/^.*(pinterest.com\/pin|pin.it)/i.test(m.text)) {
        pin(url).then(async res => {
            ...
            await m.reply(wait)
            m.reply(util.format(json))
            await this.sendFile(m.chat, json.data.url, '', 'CyberDark', m)
        }).catch(_ => _)
    }
    ...
    if (/^.*twitter.com\//i.test(m.text)) {
        twitter(url).then(async res => {
            ...
            for (let { url } of json.data) {
                this.sendFile(m.chat, url, 'ig' + (/mp4/i.test(url) ? '.mp4' : '.jpg'), 'CyberDark', m)
            }
        }).catch(_ => _)
    }
...
`.trim(), 'CyberDark', 'Audio', `.yta ${vid.url}`, 'Video', `.yt ${vid.url}`)
    }

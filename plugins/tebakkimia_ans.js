...
handler.before = async function (m) {
    ...
    if (m.quoted.id == this.tebakkimia[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.name.toLowerCase().trim()) {
            global.DATABASE.data.users[m.sender].exp += this.tebakkimia[id][2]
            await this.sendButton(m.chat, `*Benar!* +${this.tebakkimia[id][2]} XP`, 'CyberDark', 'Tebak Kimia', '.tebakkimia', m)

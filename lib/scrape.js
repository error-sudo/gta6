    ...
    return new Promise((resolve, reject) => {
        fetch('https://jagokata.com/kata-bijak/kata-' + input.replace(/\s/g, '_') + '.html?page=1')
            ...
            .then(res => {
                ...
                data.splice(2, 1)
                if (data.length == 0) return resolve({ creator: 'CyberDark', status: false })
                resolve({ creator: 'CyberDark', status: true, data })
            }).catch(reject)
    })
}
...
function facebook(url) {
    return new Promise(async (resolve, reject) => {
        ...
        axios.post(host + '/system/action.php', qs.stringify(form.data), { headers: is.headers })
            .then(({ data }) => {
                if (data.links.lenght == 0) return resolve({ creator: 'CyberDark', status: false })
                resolve({ creator: 'CyberDark', status: true, data: data.links })
            })
    })
}

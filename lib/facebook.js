...
function facebook(url) {
    return new Promise(async (resolve, reject) => {
        ...
        axios.post(host + '/system/action.php', qs.stringify(form.data), { headers: is.headers })
            .then(({ data }) => {
                if (data == 'error') return resolve({ creator: 'CyberDark', status: false, message: 'tida dapat diunduh' })
                resolve({ creator: 'CyberDark', status: true, thumb: data.thumbnail, data: data.links })
            })
    })
}
